...
final class ExportedObjectBox: @unchecked Sendable {
    ...
    init(_ object: DBusExportedObject) { self.object = object }
}
...
final class GIOExportLease: DBusExportLease, @unchecked Sendable {
    ...
    init(connection: OpaquePointer, registration: UInt32, node: UnsafeMutablePointer<GDBusNodeInfo>) {
...
nonisolated(unsafe) let gioPropertyCallback: GDBusInterfaceGetPropertyFunc = {
    ...
    guard let value = box.object.properties[name], let signature = box.object.propertySignatures[name] else { return nil }
    return try? GIOVariantCodec.make(value, signature: signature)
}
