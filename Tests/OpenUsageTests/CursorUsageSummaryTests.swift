...
final class CursorUsageSummaryMapperTests: XCTestCase {
    func testCombinesLiveEnterpriseRequestAndIndividualOnDemandShapes() throws {
        ...
        XCTAssertEqual(progress(mapped.lines, "Cursor Models")?.used, 6.25)
...
final class CursorEnterpriseProviderTests: XCTestCase {
    func testRefreshCombinesEnterpriseMetersAndStillAppendsUsageHistory() async throws {
        ...
        let modelsDescriptor = try XCTUnwrap(descriptors.first { $0.id == "cursor.cursorModels" })
        let requestTotal = try XCTUnwrap(descriptors.first { $0.id == "cursor.usage" })
        let modelsData = store.data(for: modelsDescriptor)
        let requestData = store.data(for: requestTotal)
        XCTAssertTrue(DefaultLayout.metricIDs.contains("cursor.cursorModels"))
        XCTAssertTrue(DefaultLayout.metricIDs.contains("cursor.otherModels"))
        XCTAssertFalse(DefaultLayout.metricIDs.contains("cursor.usage"))
        XCTAssertFalse(DefaultLayout.metricIDs.contains("cursor.auto"))
        XCTAssertFalse(DefaultLayout.metricIDs.contains("cursor.api"))
        XCTAssertFalse(DefaultLayout.metricIDs.contains("cursor.requests"))
        XCTAssertTrue(modelsData.hasData)
        XCTAssertEqual(modelsData.kind, .percent)
        XCTAssertEqual(modelsData.used, 6.25)
        XCTAssertTrue(requestData.hasData)
        XCTAssertEqual(requestData.kind, .count)
        XCTAssertEqual(requestData.used, 37)
        XCTAssertEqual(requestData.limit, 750)
        XCTAssertEqual(requestData.countSuffix, "requests")
